...
export function CommunityThemeCard({ theme }: { theme: CommunityTheme }) {
    ...
    <div className="group/card relative flex flex-col gap-2">
      <div className="group/preview relative flex h-56 overflow-hidden rounded-lg border">
        <div className="from-foreground/20 to-foreground-10 absolute inset-0 z-1 flex flex-col items-center justify-center gap-2 bg-gradient-to-b opacity-0 transition-opacity duration-300 ease-in-out group-hover/preview:opacity-100">
          <Button variant="outline" size="sm" className="w-28 drop-shadow" asChild>
            <Link href={`/themes/${theme.themeId}`}>View Details</Link>
          </Button>
          <Button size="sm" className="w-28 drop-shadow" onClick={handleApply}>
            Apply Theme
          </Button>
        </div>
          ...
          <div className="group relative flex flex-1">
            ...
            {swatchDefinitions.map((swatch) => (
              <div
                ...
                className={cn("group/swatch relative h-full flex-1 transition-all duration-300 ease-in-out")}
                style={{ backgroundColor: theme.styles.light[swatch.bgKey] }}
              />
            ))}
          </div>
          ...
          <div className="group relative flex flex-1">
            ...
            {swatchDefinitions.map((swatch) => (
              <div
                ...
                className={cn("group/swatch relative h-full flex-1 transition-all duration-300 ease-in-out")}
                style={{ backgroundColor: theme.styles.dark[swatch.bgKey] }}
              />
            ))}
          </div>
        </div>
      ...
      <div className="flex items-center gap-2">
          ...
          style={
            {
              ...
              "--primary-foreground": theme.styles[mode]["primary-foreground"],
        ...
        <div className="flex flex-col gap-1">
          <h3 className="line-clamp-1 text-sm leading-none font-medium">{theme.name}</h3>
          <p className="text-muted-foreground line-clamp-1 text-xs">by {theme.author.name}</p>
        </div>
